...
// 분리된 모듈들 import
import { BoxPlotProps, Margin } from "./types";
import { createBoxPlotInitialXScale, createBoxPlotInitialYScale } from "./utils";
import { useBoxPlotCanvasRendering } from "./hooks/useCanvasRendering";
import { useBoxPlotMouseInteraction } from "./hooks/useMouseInteraction";
import { useZoomConstraints } from "./hooks/useZoomConstraints";
import { BoxPlotTooltip, Tooltip } from "./components";
...
export default function BoxPlot({
  ...
  const initialXScale = useMemo(() => 
    createBoxPlotInitialXScale(data, innerWidth), [data, innerWidth]
  );
  
  const initialYScale = useMemo(() => 
    createBoxPlotInitialYScale(data, innerHeight), [data, innerHeight]
  );
  ...
  const {
    ...
    handleTooltipClick,
    handleMouseLeave,
  } = useBoxPlotMouseInteraction();

  const {
    canvasRef,
    dataPointsRef,
    updateCanvas,
  } = useBoxPlotCanvasRendering(
    width,
    height,
    margin,
    data,
  ...
  // if (!isMounted) {
  //   return (
  //     <div className={` bg-transparent p-4 flex items-center justify-center`} style={{ width: width, height: height }}>
            ...
            return (
              <div className="relative flex items-center justify-center">
                ...
                <div
                  ...
                  onClick={(event) => handleDataPointClick(event, dataPointsRef)}
                  onWheel={(event) => {
                ...
                {/* 툴팁 */}
                {(hoveredPoint || pinnedPoint) && (mousePosition || pinnedPosition) && (
                  <Tooltip
