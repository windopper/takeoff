...
`
	...
	static async createWeeklyNews(title: string, env: Env): Promise<string> {
		...
		const selectionChain = RunnableSequence.from([WEEKLY_NEWS_SELECTION_PROMPT, gemini, selectionParser]);
		...
		if (newPost) {
			const translation = await TranslateService.translateAndSaveWeeklyNewsPost(newPostId, 
                { title: newPost.title, content: newPost.content }, 'en', true);
			console.log(`Translated weekly news post ${newPost.title} to Korean: ${translation.translatedTitle}`);
		}
